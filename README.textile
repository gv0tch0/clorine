...
<pre>
   (defn get-remaining-inventory [item-sku store-number]
      ;; In the example code, this is a nested with-conneciton, which will
      ;; return the same :inventory database conneciton instance the outer
      ;; with-conneciton did in the record-restock-orders function
     (with-conneciton :inventory
